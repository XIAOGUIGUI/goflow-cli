...
let formatErrors = (list) => {
  let styleErrorResult = []
  list.forEach(function (result) {
    let warnings = []
    let fileName = result.source && result.source.split('src')[1]
    result.warnings.forEach(function (warning) {
      if (!(fileName.indexOf('.vue') > -1 && warning.rule === 'no-empty-source')) {
        warnings.push(warning)
      }
    })
    result.warnings = warnings
    if (result.warnings.length !== 0) {
      styleErrorResult.push(result)
    }
  })
  return styleErrorResult
}
/**
